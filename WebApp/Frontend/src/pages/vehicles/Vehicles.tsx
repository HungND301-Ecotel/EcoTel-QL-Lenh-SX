...
const Vehicles: React.FC = () => {
    ...
    const defaultColumns = [
        ...
        },
        {
            ...
            renderCell: (params: GridRenderCellParams<any>) => {
                ...
                return <Chip
                    sx={{ width: '120px' }}
                    label={s === StatusDeviceEnum.IN_USE ? 'Đang hoạt động' :
                        s === StatusDeviceEnum.MAINTENANCE ? 'SC; BD' :
                            s === StatusDeviceEnum.RETIRED ? 'Niêm cất' :
                                s === StatusDeviceEnum.AVAILABLE ? 'Chờ điều động' : s}
                    color={s === StatusDeviceEnum.IN_USE ? 'error' :
                        s === StatusDeviceEnum.MAINTENANCE ? 'warning' :
                            s === StatusDeviceEnum.RETIRED ? 'secondary' :
                                s === StatusDeviceEnum.AVAILABLE ? 'success' : 'default'}
                />
            },
        },
        {
    ...
    const handleOpen = (device?: any) => {
        if (device) {
            ...
            formik.resetForm();
            const defaultCategory = DeviceTypes.find(
                (p: any) => p.name === "Vận tải"
            );
            if (defaultCategory) {
        ...
        <Box>
                                ...
                                <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                                    ...
                                    <Autocomplete
                                        ...
                                        options={DeviceTypes.filter(
                                            (p: DeviceType) => p.group === DeviceTypeEnum.VEHICLE
                                        )}
            ...
            >
                ...
                <Box display="flex" alignItems={"center"}>
                    ...
                    />
                    <ListItemText
                        primary={`Chờ điều động (${allVehicles.filter((o: Device) => o.status === StatusDeviceEnum.AVAILABLE).length})`}
                        sx={{ color: "green" }}
                    />
                </Box>
                <Box display="flex" alignItems={"center"}>
                    ...
                    />
                    <ListItemText
                        primary={`Đang hoạt động (${allVehicles.filter((o: Device) => o.status === StatusDeviceEnum.IN_USE).length})`}
                        sx={{ color: "red" }}
                    />
                </Box>
                <Box display="flex" alignItems={"center"}>
                    ...
                    />
                    <ListItemText
                        primary={`SC; BD (${allVehicles.filter((o: Device) => o.status === StatusDeviceEnum.MAINTENANCE).length})`}
                        sx={{ color: "orange" }}
                    />
                </Box>
                <Box display="flex" alignItems={"center"}>
                    <Checkbox
                        color="default"
                        name="status"
                        checked={status === StatusDeviceEnum.RETIRED}
                        onChange={() => handleChange(StatusDeviceEnum.RETIRED)}
                    />
                    <ListItemText
                        primary={`Niêm cất (${allVehicles.filter((o: Device) => o.status === StatusDeviceEnum.RETIRED).length})`}
                        sx={{ color: "grey" }}
                    />
                </Box>
            </Box>
