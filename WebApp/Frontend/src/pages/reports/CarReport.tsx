    ...
    return (
        <Grid item xs={12}>
            <Paper sx={{ p: 1 }}>
                ...
                <Typography textAlign={'center'} mb={2} variant='h3' fontWeight={'bold'}>Báo cáo tổng hợp số liệu trong ca (Ô tô)</Typography>
                    ...
                    <Table stickyHeader size="small" aria-label="car-trip-report" sx={{
                        ...
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell align='center' rowSpan={2}>Cung độ thực hiện</TableCell>
                                <TableCell align='center' rowSpan={2}>Chiều cao nâng tải (m)</TableCell>
                                <TableCell align='center' colSpan={3}>Sản lượng</TableCell>
                        ...
                        <TableBody>
                            {data.map((item: any, idx: number) => {
                                ...
                                return reps.map((r: any, i: number) => {
                                    const mats = r.materials?.length ? r.materials : [{ material: "" }];
                                    ...
                                    return mats.map((m: any, mIdx: number) => (
                                        <React.Fragment key={`${item._id}-${i}-${mIdx}`}>
                                            ...
                                            <TableRow>
                                                ...
                                                {i === 0 && mIdx === 0 && (
                                                    <>
                                                        <TableCell rowSpan={totalMaterials * 2} align="center">{idx + 1}</TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} sx={{ whiteSpace: "pre-line", minWidth: 200 }}>
                                                            {(item.assignedTo || []).map((u: any) => u?.fullName).join('\n')}
                                                        </TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} sx={{ whiteSpace: "pre-line", minWidth: 80 }}>
                                                            {(item.assignedTo || []).map((u: any) => u?.salaryCode).join('\n')}
                                                        </TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} sx={{ whiteSpace: "pre-line", minWidth: 80 }}>
                                                            {(item.device || []).map((i: any) => i).join('\n') || ''}
                                                        </TableCell>
                                                    </>
                                                )}
                                                ...
                                                {mIdx === 0 && (
                                                    <TableCell rowSpan={mats.length * 2} sx={{ minWidth: 100 }}>
                                                        {r.excavator || ''}
                                                    </TableCell>
                                                )}
                                                {mIdx === 0 && (
                                                ...
                                                {/* Vật liệu */}
                                                <TableCell rowSpan={2} sx={{ minWidth: 80 }} align="center">
                                                    {m.material?.name || ''}
                                                </TableCell>

                                                {/* --- Cung độ (tổng) --- */}
                                                <TableCell align="center" sx={{ minWidth: 80 }}>{m.totalDistance || 0}</TableCell>
                                                <TableCell align="center" sx={{ minWidth: 80 }} rowSpan={2}></TableCell>
                                                <TableCell align="center" sx={{ minWidth: 80 }} rowSpan={2}></TableCell>
                                                <TableCell align="center" sx={{ minWidth: 80 }}>{m.count || 0}</TableCell>
                                                <TableCell align="center" sx={{ minWidth: 80 }} rowSpan={2}></TableCell>
                                                {i === 0 && mIdx === 0 && (
                                                    <>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ minWidth: 80, whiteSpace: "pre-line", }}>{(item?.fuelRemain || []).map((i: any) => i).join('\n')}</TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ minWidth: 80, whiteSpace: "pre-line", }}>{(item?.fuelReceived || []).map((i: any) => i).join('\n')}</TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ minWidth: 80, whiteSpace: "pre-line", }}>{(item?.fuelRemainEnd || []).map((i: any) => i).join('\n')}</TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ minWidth: 80, whiteSpace: "pre-line", }}>{(item?.fuelRemainUsed || []).map((i: any) => i).join('\n')}</TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ whiteSpace: "pre-line", minWidth: 80 }}></TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ whiteSpace: "pre-line", minWidth: 80 }}></TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ whiteSpace: "pre-line", minWidth: 80 }}></TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ whiteSpace: "pre-line", minWidth: 80 }}>{(item?.travelHours || []).map((i: any) => i).join('\n')}</TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ whiteSpace: "pre-line", minWidth: 80 }}></TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ whiteSpace: "pre-line", minWidth: 80 }}></TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ whiteSpace: "pre-line", minWidth: 80 }}></TableCell>
                                                        <TableCell rowSpan={totalMaterials * 2} align='center' sx={{ whiteSpace: "pre-line", minWidth: 80 }}></TableCell>
                                            ...
                                            <TableRow>
                                                <TableCell sx={{ whiteSpace: "pre-line" }} align="center">
                                                    {(m.distances || [])
                                                ...
                                                <TableCell sx={{ whiteSpace: "pre-line" }} align="center">
                                                    {(m.distances || [])
                                                        .map((d: number, idx: number) => `${new Date(m.times[idx]).toLocaleTimeString('vi-VN')}`)
                                                        .join("\n")}
                                                </TableCell>
                                            </TableRow>
                                        </React.Fragment>
