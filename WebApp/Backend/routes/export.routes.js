...
async function buildVehicle(order, workbook) {
  ...
  const sheetName = `${order.assignedTo?.username}_${formatDate(
    order.workingDate
  )}_${order.shift?.name}_${order._id}`
    .replace(/[\\\/:*?\[\]]/g, "-")
    .substring(0, 31);
  ...
  worksheet.getCell(`C${nextRow + 3}`).value =
  ...
  worksheet.getCell(`J${rowHeader1 + 1}`).value =
    "Trọng lượng \n tạm tính \n (tấn)";
  worksheet.getCell(`K${rowHeader1 + 1}`).value =
    "Sản lượng \n tạm tính \n(tkm)";
  ...
  worksheet.mergeCells(
    `I${totalRow + 6 + deviceRow}:L${totalRow + 6 + deviceRow}`
  );
  worksheet.getCell(`I${totalRow + 6 + deviceRow}`).value = "NGƯỜI RA LỆNH";
  worksheet.getCell(`I${totalRow + 6 + deviceRow}`).font = { bold: true };
  worksheet.getCell(`I${totalRow + 6 + deviceRow}`).alignment = {
    horizontal: "center",
    vertical: "middle",
  };
  if (order.createdBy?.signature) {
    const response = await axios.get(order.createdBy.signature, {
    ...
    worksheet.mergeCells(
      `J${totalRow + 7 + deviceRow}:K${totalRow + 9 + deviceRow}`
    );
  ...
  worksheet.mergeCells(
    `I${totalRow + 10 + deviceRow}:L${totalRow + 10 + deviceRow}`
  );
  worksheet.getCell(`I${totalRow + 10 + deviceRow}`).font = { bold: true };
  worksheet.getCell(`I${totalRow + 10 + deviceRow}`).alignment = {
    horizontal: "center",
    vertical: "middle",
  };
  worksheet.getCell(`I${totalRow + 10 + deviceRow}`).value =
...
async function buildExcavator(order, workbook) {
  ...
  grouped.forEach((g, i) => {
    ...
    g.materials.forEach((m) => {
      ...
      if (rowIndexTrip - 1 > startRowTrip) {
        ["A", "B"].forEach((col) => {
          worksheet.mergeCells(
            `${col}${startRowTrip}:${col}${rowIndexTrip - 1}`
          );
  ...
  worksheet.mergeCells(
    `I${totalRow + 6 + deviceRow}:J${totalRow + 6 + deviceRow}`
  );
  worksheet.getCell(`I${totalRow + 6 + deviceRow}`).value = "NGƯỜI RA LỆNH";
  worksheet.getCell(`I${totalRow + 6 + deviceRow}`).font = { bold: true };
  worksheet.getCell(`I${totalRow + 6 + deviceRow}`).alignment = {
  ...
  if (order.createdBy?.signature) {
    ...
    worksheet.mergeCells(
      `I${totalRow + 7 + deviceRow}:J${totalRow + 9 + deviceRow}`
    );
  ...
  worksheet.mergeCells(
    `I${totalRow + 10 + deviceRow}:J${totalRow + 10 + deviceRow}`
  );
  worksheet.getCell(`I${totalRow + 10 + deviceRow}`).font = { bold: true };
  worksheet.getCell(`I${totalRow + 10 + deviceRow}`).alignment = {
    horizontal: "center",
...
async function buildMaintence(order, workbook) {
  ...
  worksheet.getCell(`I${fuelEndRow + 2}`).alignment = {
    horizontal: "center",
    vertical: "middle",
  };
  if (order.createdBy?.signature) {
    ...
    try {
      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        for (const ca of shiftList) {
          ...
          infoRow.value = `Đơn vị: ${dep?.code}                  Ca: ${
            ca.name
          }                  , ngày:    ${formatDate(
            d
          )}                             Tên cán bộ: ${req.user?.fullName}`;
      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        for (const ca of shiftList) {
          ...
          if (signature) {
            ...
            worksheet.mergeCells(
              `${startSignature}${currentRow + 4}:${endSignature}${
                currentRow + 7
              }`
            );
        ...
        for (const ca of shiftList) {
            ...
            order.reports.forEach((rep) => {
              ...
              rep.trips.forEach((trip) => {
                if (trip.material) {
                  // tạo key duy nhất theo _id hoặc name
                  const key =
                    trip.material._id?.toString() || trip.material.name;
          ...
          result.forEach((item, idx) => {
            ...
            reps.forEach((r, i) => {
              ...
              materials.forEach((m, mIdx) => {
                row1.getCell(8 + maxTrips + mIdx).value =
                  r.summary?.[m?.name]?.distance || 0;
              });

              // ✅ Tổng chuyến: cũng gộp 2 hàng
              row1.getCell(8 + maxTrips + materials.length).value =
                r.totalDistance || 0;
              currentRow++;
              ...
              worksheet.mergeCells(
                `${row2.getCell(totalCol).address}:${
                  row3.getCell(totalCol).address
                }`
              );
    ...
    try {
      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        for (const ca of shiftList) {
          ...
          const formattedData = filteredOrders.flatMap((order, orderIndex) => {
            if (!order.shiftReport || !order.shiftReport.vehicleReports)
              return [];
    ...
    try {
      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        for (const ca of shiftList) {
          ...
          worksheet.getCell("A1").value = `Ca: ${ca.name}, ngày: ${formatDate(
            d
          )}         Tên cán bộ: ${req.user?.fullName}`;
          ...
          worksheet.getCell(`C${length + 7}`).alignment = {
            horizontal: "center",
            vertical: "middle",
          };
          ...
          worksheet.getCell(`H${length + 7}`).alignment = {
            horizontal: "center",
            vertical: "middle",
          };
    ...
    try {
      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        for (const ca of shiftList) {
          ...
          const infoRow = worksheet.getCell("A1");
          infoRow.value = `Ca: ${ca.name}, ngày: ${formatDate(
            d
          )}         Tên cán bộ:`;
          ...
          worksheet.getCell(`E${length + 7}`).alignment = {
            horizontal: "center",
            vertical: "middle",
          };
    ...
    try {
      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        for (const ca of shiftList) {
          ...
          for (const order of orders) {
            const sheetName = `${formatDate(d)}_${ca.name}_${
            ...
            addRow(
              [
                `Ca: ${ca?.name}     Ngày: ${d.getDate()}     Tháng: ${
                  d.getMonth() + 1
                }     Năm: ${d.getFullYear()}`,
              ],
              {
            ...
            addRow(
              [
                `Người giao: ${order?.createdBy?.fullName}       Người nhận: ${order?.assignedTo?.fullName}`,
              ],
              {
                merge: "A2:C2",
              }
            ...
            addRow(
              [
                "Tuyến hoạt động: .........................  Cự ly (km): .............  Chở than/đất: ..........................",
              ],
              {
                merge: "A5:C5",
              }
            ...
            addRow(
              [
                "Số chuyển theo định mức: ..............  Số chuyển thực tế: ..............  Giờ hoạt động: ...............",
              ],
              {
                merge: "A6:C6",
              }
            ...
            addRow(
              [
                "Giờ sau cộng tiếp sau KT: .............   Giờ cộng tiếp sau bảo dưỡng cấp 1: ..............",
              ],
              {
                merge: "A7:C7",
              }
            ...
            addRow(
              [
                "Tồn đầu ca: ................   Lĩnh trong ca: ...............   Tồn cuối ca: ...............",
              ],
              {
                merge: "A17:C17",
              }
            ...
            addRow(
              [
                "Mức tiêu hao: theo định mức: ...............  thực tế: ...............  quá mức: ...............",
              ],
              {
                merge: "A18:C18",
              }
            ...
            addRow(
              [
                "Bổ sung dầu nhớt: loại dầu: ...............  vị trí: ...............  số lượng: ...............",
              ],
              {
                merge: "A19:C19",
              }
            ...
            addRow(["IV. Tình trạng kỹ thuật xe khi giao lại cho ca sau:"], {
              bold: true,
            });
            ...
            addRow(["(Ký, ghi rõ họ tên)", "", "", "(Ký, ghi rõ họ tên)"], {
              bold: true,
            });
            ...
            addRow(["NGƯỜI NHẬN CA", "", "", "NGƯỜI RA LỆNH HOẠT ĐỘNG"], {
              bold: true,
            });

            addRow(["(Ký, ghi rõ họ tên)", "", "", "", "CA TIẾP THEO"], {
              bold: true,
            });
      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        for (const ca of shiftList) {
          ...
          for (const order of orders) {
            ...
            addRow(
              [
                `Người bàn giao: ${order?.createdBy?.fullName}       Người nhận bàn giao: ${order?.assignedTo?.fullName}`,
              ],
              {
                merge: "A2:C2",
              }
            ...
            addRow(
              [
                "- Số lượng thiết bị huy động/tổng số ............................  bao gồm:",
              ],
              {
                merge: "A5:C5",
              }
            ...
            addRow(["- Các thiết bị kiểm tu, bảo dưỡng bao gồm:"], {
              merge: "A8:C8",
            });
            worksheet.addRow([generateDotLine(200, "")]);
            worksheet.addRow([generateDotLine(200, "")]);
            ...
            worksheet.addRow([
              generateDotLine(178, "+ Bảo dưỡng cấp 3(hoặc 1000 giờ): "),
            ]);
            worksheet.addRow([
              generateDotLine(183, "+ Bảo dưỡng cấp 2000 giờ: "),
            ]);
            worksheet.addRow([
              generateDotLine(185, "- Các thiết bị dừng để TĐT: "),
            ]);
            worksheet.addRow([
              generateDotLine(180, "- Các thiết bị sửa chữa đột suất lớn: "),
            ]);
            worksheet.addRow([generateDotLine(200, "")]);
            worksheet.addRow([generateDotLine(200, "")]);
            worksheet.addRow([
              generateDotLine(180, "- Các thiết bị dừng do các lý do khác: "),
            ]);
            worksheet.addRow([generateDotLine(200, "")]);
            worksheet.addRow([
              generateDotLine(
                175,
                "- Nội dung công việc đã thực hiện trong ca: "
              ),
            ]);
            worksheet.addRow([generateDotLine(200, "")]);
            worksheet.addRow([generateDotLine(200, "")]);
            worksheet.addRow([generateDotLine(200, "")]);
            worksheet.addRow([generateDotLine(200, "")]);
            addRow(["II- Nội dung công việc giao lại ca sau:"], {
              bold: true,
              merge: "A31:C31",
            });
            worksheet.addRow([generateDotLine(200, "")]);
            ...
            worksheet.addRow([generateDotLine(200, "")]);
            worksheet.addRow([generateDotLine(200, "")]);
            addRow(["III- Dự báo nguy cơ mất an toàn:"], {
              bold: true,
              merge: "A36:C36",
            });
            worksheet.addRow([generateDotLine(200, "")]);
